...
- In the terminal:
    ...
    Selection an option: 1
    Selection an option: 1
    Selection a opton: 1 (Provide a one-off ..)
    python manage.py migrate
    python manage.py runserver
    ```
...
- Fix in crm/leads/view.py/ 50 - 4:40 Error at creating a lead in `http://127.0.0.1:8000/leads/create`, after clicking on submit
    ```python
    class LeadCreateView(OrganizerAndLoginRequiredMixin, CreateView):
        template_name = "leas/lead_create.html"
        form_class = LeadModelForm

        def get_success_url(self):
            return reverse("leads:lead-list")
        def form_valid(self, form):
            lead = form.save(commit=False)  #added this
            lead.organization = self.request.user.userprofile  #added this
            lead.save() #added this
            send_email(
                subject="A lead has been created',
